 ...
 *              (dma_callback: zero-transfer success, 
 *				0:   full transfer complete success
 *				1:   half transfer complete success
 *			      < 0: error happens).
 */
struct dma_config {
	u32_t  dma_slot :             6;
 ...
 * @retval Negative errno code if failure.
 */
static inline int dma_transfer_config(struct device *dev,
...
static inline enum dma_burst_length dma_width_index(u32_t size)
{
	...
	if (size < 1 || size > 32) {
		return BURST_TRANS_LENGTH_1; /* Zero is the default (8 Bytes) */
	}
	...
	return (enum dma_burst_length)find_msb_set(size);
}
...
static inline enum dma_burst_length dma_burst_index(u32_t burst)
{
	...
	if (burst < 1 || burst > 256) {
		return BURST_TRANS_LENGTH_1; /* Zero is the default (1 burst length) */
	}
	...
	/* Convert to bit pattern for writing to a register */
	return (enum dma_burst_length)find_msb_set(burst);
}
