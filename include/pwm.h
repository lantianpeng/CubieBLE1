...
typedef int (*pwm_pol_set_t)(struct device *dev, u32_t pwm,
			     u8_t pol);
...
static inline int pwm_pol_set(struct device *dev, u32_t pwm,
				     u8_t pol)
{
	struct pwm_driver_api *api;

	api = (struct pwm_driver_api *)dev->driver_api;
	return api->pol_set(dev, pwm, pol);
}
