...
static u32_t rmcTimeoutValue[ID_NUM_TIMEOUTS] = {
	...
	K_SECONDS(CONFIG_RMC_SEC_TIMEOUT), /* 6s */
#ifdef CONFIG_IRC_SW_RX
	K_SECONDS(CONFIG_RMC_LEARN_KEY_TIMEOUT), /* 5s */
#endif
};
...
void system_app_timer_event_handle(u32_t timer_event)
{
	...
	break;
	case ID_RMC_SEC_TIMEOUT:
		SYS_LOG_INF("sec timeout");
		rmc_sm_execute(RMC_MSG_TIMER_SEC_TIMEOUT);
	break;
#ifdef CONFIG_IRC_SW_RX
	case ID_IR_LEARN_KEY_TIMEOUT:
		SYS_LOG_INF("ir learn key timeout");
		void enter_into_learn_mode(void);
		enter_into_learn_mode();
	break;
#endif
	default:
